...
class Cell:
    ...
    def __str__(self):
        return f"Клетка выглядит {self.quantity_cell * '*'}"

    def __add__(self, other):
        return f"Происходит операция сложения. {Cell(self.quantity_cell + other.quantity_cell)}"

    def __sub__(self, other):
        cell_end = self.quantity_cell - other.quantity_cell
        if cell_end > 0:
            return f"Происходит операция вычитания. {Cell(self.quantity_cell - other.quantity_cell)}"
        else:
            return f"Результат вычитания клеток отрицательный."

    def __mul__(self, other):
        return f"Происходит операция умножения. {Cell(self.quantity_cell * other.quantity_cell)}"

    def __truediv__(self, other):
        return f"Происходит операция деления. {Cell(round(self.quantity_cell // other.quantity_cell))}"

    def make_order(self, cell_in_row):
        row = ""
        for i in range(int(self.quantity_cell / cell_in_row)):
            row += f'{"*" * cell_in_row}\\n'
        row += f'{"*" * (self.quantity_cell % cell_in_row)}'
        return row
